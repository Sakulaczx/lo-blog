...
class Article(models.Model):
    ...
    category = models.ForeignKey(
        'Category', verbose_name=u'模块', on_delete=models.CASCADE)
    ...
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=u'作者', on_delete=models.CASCADE)
...
class MindMap(models.Model):
    ...
    data = models.TextField(null=False, blank=False)  # KittyMind的JSON数据
    group = models.ManyToManyField(Group, blank=True)  # "文章所属组id，通过逗号
    title = models.CharField(max_length=200, blank=False, null=False, default='new mindmap')  # map标题
    image = models.ManyToManyField(
        'MindMapImage', blank=True)  # 脑图中所有使用本地图片的信息的image_id
    modify_user = models.IntegerField(blank=False, null=False, default=-1)
    create_user = models.IntegerField(blank=False, null=False, default=-1)
    category_id = models.IntegerField(blank=True, null=True)
    modify_time = models.DateTimeField(auto_now=True)  # 修改时间
    create_time = models.DateTimeField(null=False, auto_now_add=True)  # 创建时间
    lock_user_id = models.IntegerField(blank=True, null=True)
    lock_time = models.IntegerField(blank=True, null=True)


class MindMapImage(models.Model):
    filename = models.CharField(max_length=30, null=False)  # 文件名
